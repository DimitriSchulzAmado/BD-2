...
class BookModel:
    ...
    def create_book(self, title: str, autor: str, year: int, price: float):
        try:
            book = {"titulo": title, "autor": autor, "ano": year, "preco": price}
            res = self.db.collection.insert_one(book)
    ...
    def update_person(self, id: str, title: str, autor: str, year: int, price: float):
        try:
            res = self.db.collection.update_one({"_id": ObjectId(id)}, {"$set": {"titulo": title, "autor": autor, "ano": year, "preco": price}})
